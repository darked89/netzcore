...
postscript(paste(dirname, "2a.eps", sep=""))
d<-read.table(paste(dirname, 'all_vs_all/auc_phenotypes.dat', sep=""))
boxplot(d, xlab="Prediction method", ylab="Average AUC over all diseases (%)", names=c("NetScore", "NetZcore", "NetShort", "Func. Flow", "ToppGene"), col=rainbow(5), notch=FALSE, varwidth=FALSE)
dev.off()

postscript(paste(dirname, "2b.eps", sep=""))
d<-read.table(paste(dirname, 'all_vs_all/cov_phenotypes.dat', sep=""))
boxplot(d, xlab="Prediction method", ylab="Ratio of correctly predicted proteins in the highest ranking top 10% proteins (%)", names=c("NetScore", "NetZcore", "NetShort", "Func. Flow", "ToppGene"), col=rainbow(5), notch=FALSE, varwidth=FALSE)
dev.off()

postscript(paste(dirname, "3a.eps", sep=""))
...
postscript(paste(dirname, "3b.eps", sep=""))
...
postscript(paste(dirname, "3c.eps", sep=""))
...
postscript(paste(dirname, "4.eps", sep=""))
d<-read.table(paste(dirname, 'all_vs_all/auc_phenotypes.dat', sep=""))
barplot(as.matrix(t(d)), beside=T, xlab="Prediction method", ylab="Average AUC over all diseases (%)", names.arg=c("Biana", "Bia. filtered", "Entrez", "Goh", "Bia. filtered weighted"), col=rainbow(5), legend.text=c("NetScore","NetZcore", "NetShort", "Func. Flow", "ToppGene"))
dev.off()

postscript(paste(dirname, "5.eps", sep=""))
