...
def create_edge_scores_file_from_node_scores(network_file, node_scores_file, ignored_seed_nodes = None, default_non_seed_score = 0):
    """
...
def create_node_scores_file(network_file, network_file_identifier_type, node_biana_file, association_scores_file, association_scores_file_identifier_type, node_scores_file, ignored_seed_nodes = None, default_non_seed_score = 0):
    """
    ...
    node_to_genes, gene_to_nodes = biana_output_converter.get_attribute_to_attribute_mapping(node_biana_file, network_file_identifier_type, association_score_file_identifier_type)
    ...
    print "Covered gene products (seed nodes):", len(seeds), "among", len(g.number_of_nodes())
    return 
...
def create_degree_filtered_network_file(network_file, network_file_filtered, degree):
    ...
    g = network_utilities.filter_network(g, degree) 
    # Get degrees of highly connected nodes
    network_utilities.analyze_network(g)
    output_network_in_sif(g, network_file_filtered)
    return
...
def create_human_interactome_using_biana(node_file_prefix="human_nodes", network_files_prefix="human_network", network_type="experimental", load_from_saved_session = False): 
	...
	relation_attributes.append("stringscore")
