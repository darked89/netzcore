...
run_random_walk_with_restart<-function(node_file, edge_file, out_file, propagation=FALSE) {
    A<-efficient_adj_matrix(edge_file)
    p_0<-eff_node_scores(node_file)
...
if(F){ # quit non-efficient functions
...
## efficient version of get_adjacency_matrix     # ABere 2012
efficient_adj_matrix<-function(edge_file){
library(igraph)
d<-read.table(edge_file)
ids<-unique(c(as.vector(d[,1]),as.vector(d[,3])))
ids<-sort(ids)
gd<-graph.edgelist(as.matrix(d[,c(1,3)]),directed=FALSE)
gd<-set.edge.attribute(gd,"weight",value=d[,2])
avv<-get.adjacency(gd)
av<-avv[ids,ids]
return(av)
}

## efficient version of get_node_scores   # ABere 2012
eff_node_scores<-function(node_file) {
...
## efficient normalization                # ABere 2012
efficient_normalization<-function(x){
w<-colSums(x)
CS<-as.matrix(w)
aux<-CS%*%t(CS)
ww<-sqrt(aux)
...
random_walk_with_restart <- function(A, p_0, r, max_n_iter, convergence_cutoff, propagation=FALSE) {
	...
	if(F){							  ## voided the double loop and replace it by a matrix multiplication (efficient_normalization function) 	ABere  2012
	...
	W<-efficient_normalization(A)        ## ABere 2012
